    ...
    while not done:
        ...
        while True:
            ...
            if s[0] < min_x or s[1] + 1 > max_y:
                print(f"[1]: {len(fallen_sand)}")
                done = True
                break
            # Keep falling down
            if (s[0], s[1] + 1) not in (rocks | fallen_sand):
                s = (s[0], s[1] + 1)
                continue
            # Check if we can go left
            if (s[0] - 1, s[1] + 1) not in (rocks | fallen_sand):
                s = (s[0] - 1, s[1])
                continue
...
def two(rocks):
    ...
    floor = max_y + 2
    # Add the floor
    while (500, 0) not in fallen_sand:
        s = (500, 0)
        while True:
            # Keep falling down
            if (s[0], s[1] + 1) not in (rocks | fallen_sand) and s[1] + 1 != floor:
                s = (s[0], s[1] + 1)
                continue
            # Check if we can go left
            if (s[0] - 1, s[1] + 1) not in (rocks | fallen_sand) and s[1] + 1 != floor:
                s = (s[0] - 1, s[1])
                continue
            # Check if we can go right
            if (s[0] + 1, s[1] + 1) not in (rocks | fallen_sand) and s[1] + 1 != floor:
            ...
            fallen_sand.add(s)
            break
        print(f"Sand count: {len(fallen_sand)}")
    print(f"[2]: {len(fallen_sand)}")
